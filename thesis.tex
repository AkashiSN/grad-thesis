...
\documentclass[autodetect-engine, dvipdfmx-if-dvi, a4paper, ja=standard, 10pt, textwidth-limit=45, openany]{bxjsbook}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=50mm]{fig/fqdn_structure-crop.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=90mm]{fig/icn_url_structure-crop.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=140mm]{fig/table-crop.pdf}
...
このデータには重複が含まれているので重複を削除し，これは60GBほどのサイズで約8.8億件のURLを含み，全URLリストと呼ぶ．
各CRでの実際的なURLは10MBほどであるという仮定のうえ，ランダムな10MBを抽出し，解析データとした．
ここには約14万件のURLが含まれる．
...
そのセクションの文字数が3文字未満の場合はセクションの文字数に応じて３文字にする(パディング)．

\begin{description}
  \item[セクションが1文字のとき] ICNの長さとスラッシュの数を掛けたものをuint16型で付加する
  \item[セクションが2文字のとき] ICNのスラッシュの数をbyte型として付加する
\end{description} 

次に，各セクションから前3文字を抜き出して配列headsとする．
同様に後3文字を抜き出して配列tailsとする．ただしパティングが含まれているセクションはパディングと元の文字との順序を入れ替える．
結果的にheadsとtailsのどちらかに含まれるセクションの数が3未満だったらそのURLは処理の対象外とする．

\begin{figure}[h]
  \centering
  \includegraphics[width=135mm]{fig/hash_separate-crop.pdf}
...
\section{解析プログラム}
Golangにより作成したプログラムの概要を述べる．

\subsection{前処理}
\subsubsection{重複削除}
入力されたデータの重複を削除する．今回は121GBのデータなのでlinuxのuniqueコマンドでは時間がかかりすぎるため実装した．
文字数による分割統治法を用いて，分割したデータごとに並行処理を行うことで重複削除の高速化を図った．
重複削除したら60GB程度になった．
\subsubsection{サンプル抽出}
重複のないデータから10MBをランダムに抽出する機能を実装した．
...
バッファーに入るだけ読み込んでから並列プロセスに渡して以下の処理をする．
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.65]{graph/sampled_data/sampled_data-crop.pdf}
...
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.65]{graph/ratio_of_etld/ratio_of_etld_all-crop.pdf}
  \caption{Probability of eTLD appearance in the sampled URLs．}
  \label{graph:ratio_of_etld_all}
\end{figure}
\begin{figure}[p]
  \centering
  \includegraphics[scale=0.65]{graph/ratio_of_etld/ratio_of_etld_detail-crop.pdf}
...
累積分布と1との差の絶対値を相補累積分布と呼ぶ(Complementary Cumulative Distribution Function: CCDF)．
これをFig. \ref{graph:hash_ccdf_A-C}に示す．
このグラフではある横軸において値が小さい方がより衝突の割合が少ない．
...
グラフよりeTLDが\url{me}のとき衝突しない確率が低くなっていることがわかる．
\url{formspring.me}というサイトがデータの大半を占めており，\url{icn:/me/formspring/nandaghizzo/q/908453668}のようなICN-URLの形式である．
...
eTLDのみだと母数が大きいので細分化するために次の3種の規則によりRootをeTLDに追加する．
\begin{enumerate}
...
ので上記1の規則について詳しく見るためにFig. \ref{graph:ratio_of_pointer_detail}とTable \ref{tab:ratio_of_pointer_detail}に出現確率の多い上位10件のポインタを示す．
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.6]{graph/ratio_of_pointer/ratio_of_pointer_all-crop.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.6]{graph/ratio_of_pointer/ratio_of_pointer_detail-crop.pdf}
...
ポインタテーブルを併用することにより5回以上の衝突確率を0.3\%に抑えることができる．
...
\begin{enumerate}
  \item 3通りのハッシュアルゴリズムを提案した．ICN-URLを"/"で分割し，3バイトの文字列を切り出し，
      そのうちの2--3の文字列どおしのXOR演算から3バイトから4バイトのハッシュ値を生成した．
  ...
  そのため衝突率が大きくなっていた．
  \item eTLDにRootを加えることで偏りが大きかった\url{com}の占める割合を24\%に下げることで衝突確率を0.3\%に抑えることができた．
\end{enumerate}
