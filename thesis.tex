...
ハードウェア実装に向けた課題は，TCP/IPにおけるIPアドレスの代わりにコンテンツ名を用いて名前解決とルーティングを行うためCRでの処理や，各テーブルに必要な記憶容量も増加するという点である．

\section{本研究の目的}
上記課題を解決するためにコンテンツ名に対してハッシュを用いたり，各テーブルにおいて新たなデータ構造を考案することである．

\paragraph{想定するハードウェアアーキテクチャ及びネットワーク}
NetFPGA SUME\footnote{\url{https://netfpga.org/site/#/systems/1netfpga-sume/details/}}の利用を前提として，まず10Gbps,平均パケットサイズ800 Byteの場合ボトルネックの許容平均処理遅延を求める．

許容できるボトルネックの平均処理遅延$t\ [\mathrm{sec/packet}]$は，パケットあたりの(見かけの)処理時間と考えられる．
これは，目標とするスループット$v\ [\mathrm{bit/sec}]$，平均パケットサイズを$s\ [\mathrm{bit/packet}]$とすると，$t=s/v$で表される．
ここにパラメータ$v=10\ \mathrm{Gbps}=10^{10}\ [\mathrm{bit/sec}],\ s=800\ [\mathrm{Byte}] = 6400\ [\mathrm{bit}]$を当てはめると，
$t=6400/10^{10}=640\times 10^{-9}\ [\mathrm{sec/packet}]$であり，許容平均処理遅延は$640\ [\si{\nano\second}]$となる．
